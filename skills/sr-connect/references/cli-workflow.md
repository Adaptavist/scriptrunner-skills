...
3. When they say it is done, confirm `allowed: true` on that entry before running again: `egress-firewall list`, or, for a hostname the user did not add and cannot list, `egress-firewall add-fetch-destination` with it again, which answers with its current state.
...
- `egressFirewall.allowed: false` after `api-connection create` or `update`: the attached connector is blocked. `state` and `reason` name the cause; see Adding entries.
