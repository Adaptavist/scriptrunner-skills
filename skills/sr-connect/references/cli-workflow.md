...
3. Start from the safest environment. Add the others once the work is confirmed, unless the user asks for them up front.
...
1. `event-listener get -e <env> <listenerId>` for `webhookUrl`, `setupUrl` and `connectorId`. Both URLs are per environment.
