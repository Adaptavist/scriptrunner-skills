# Troubleshoot Script Manager

- Platform: cloud
- Space: SR4JC
- Hierarchy: features > script-manager
- Doc ID: doc-sr4jc-480378914
- Source: https://docs.adaptavist.com/sr4jc/latest/features/script-manager/troubleshoot-script-manager

Saved _Script Manager_ scripts are available for reuse in Groovy and Jira Expression Language code editors **only** and are not available for use with Behaviours.
...
-   Class was not found due to a missing import.
...
You may notice this as a missing script or a missing method exception.
...
Some level of failure can occur during autocompletion as we fetch code from remote storage and compile on the fly, so if a saved script appears to be missing according to the autocompletion but you know it's there, you can fall back to writing the import statement manually. 
...
We have built Script Manager to handle hundreds of scripts, even thousands. However, Cloud environments can have drawbacks when operating at such scales, and various limiting conditions can occur depending on variations like script size, script count, name length, and others. We recommend that if you use a large number of saved scripts, then it should only be considered as an extreme case, and efforts should be made to prevent the code base from reaching such an extent.

### What happens if a JEL file is accidentally deleted?

If a `.jel` file is deleted by accident, the outcome depends on where it is used:

-   Script Listener condition: An error will occur when the condition is evaluated.
-   Workflow Restrict Transitions or Validate Details: These will not run, and an error will occur, as deleting the file also removes the expression stored by Atlassian.
