...
- Doc ID: doc-sr4js-484576996
- Source: https://docs.adaptavist.com/sr4js/latest/scriptrunner-migration/migrating-to-cloud/troubleshoot-scriptrunner-migration

Feature Differences

ScriptRunner for Jira Cloud **does not have the same feature set** as the Server/Data Center version. Our [Feature Parity table](https://docs.adaptavist.com/sr4js/latest/scriptrunner-migration/migrating-to-cloud/feature-parity-and-script-alternatives) provides information about each individual ScriptRunner for Server/Data Center feature's parity. We also provide details on workaround script/function alternatives where there is currently no parity with Cloud.
...
-   The filter contains nested functions such as, `issueFunction in linkedIssuesOf(issueFunction in epicsOf(project=TEST))`
    ...
    -   ScriptRunner Enhanced Search for Jira Cloud does not currently support nested Enhanced Search functions, as outlined in our [Feature Parity table](https://docs.adaptavist.com/sr4js/latest/scriptrunner-migration/migrating-to-cloud/feature-parity-and-script-alternatives). However, if you would like to request this feature, you can add your request to our nolt board [here](https://scriptrunner-for-jira-cloud.nolt.io/ "https://scriptrunner-for-jira-cloud.nolt.io/").
        
-   The filter contains any Enhanced Search functions or keywords that are not compatible with the Cloud version.
    
    -   Refer to our [Feature Parity table](https://docs.adaptavist.com/sr4js/latest/scriptrunner-migration/migrating-to-cloud/feature-parity-and-script-alternatives) for details on which specific functions or keywords are not compatible.
...
![Image of JQL error](/sr4js/files/latest/484576996/484577001/1/1765569650000/error+message+field+issue1.png)
...
![Image of enhanced search](/sr4js/files/latest/484576996/484576998/1/1765569650000/troubleshoot+migrate.png)
...
If you have yet to complete migrating from ScriptRunner for Jira for Server/DC to Cloud using the [Jira Cloud Migration Assistant](https://support.atlassian.com/migration/docs/jira-cloud-migration-assistant/), when you navigate to **ScriptRunner → Migration Reports,** you are presented with a landing screen that links to our product documentation.  
![Migration reports image](/sr4js/files/latest/484576996/484576999/1/1765569650000/check+out+your+migration+reports.jpeg)

Once you have completed your migration from ScriptRunner for Jira for Server/DC to Cloud using the [Jira Cloud Migration Assistant](https://support.atlassian.com/migration/docs/jira-cloud-migration-assistant/), you can view a migration report that provides information about any items that failed to migrate and the reason for failure.

1.  Navigate to **ScriptRunner → Migration Reports**.
2.  Click on **Migration Reports** in the left-hand menu of your ScriptRunner for Jira Cloud instance and the _Migration Reports_ screen appears.![Image of migration reports page ](/sr4js/files/latest/484576996/484577000/1/1765569650000/migration+report+list.jpg)
...
It may be the case that a ScriptRunner Server/Data Centre [JQL function](https://docs.adaptavist.com/spaces/SR4JC/pages/153914344/JQL+Query+Comparison) does not have [feature parity](https://docs.adaptavist.com/sr4js/latest/scriptrunner-migration/migrating-to-cloud/feature-parity-and-script-alternatives) in ScriptRunner Cloud so part of that particular query doesn't work in Cloud. 
